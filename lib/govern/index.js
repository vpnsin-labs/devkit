...
async function cmdDoctor(ctx) {
  ...
  try {
    ...
    if (scopes != null) log.info(`token scopes: ${scopes || '(fine-grained or App token)'}`);
    log.info(`org: ${ctx.org}`);
    log.info('Projects v2 needs a PAT/App token with org Projects read/write (GITHUB_TOKEN will NOT work).');
  } catch (err) {
