...
${c.bold('Commands:')}
  ...
  ${c.cyan('azure <command>')}          Azure Repos: branch policies, repo creation, doctor — see devkit govern azure --help
...
export async function runGovern(argv) {
  ...
  // Azure Repos governance lives in its own namespace (fetch-based, no Octokit) and
  // owns its own --help, so dispatch it before the GitHub help check.
  if (command === 'azure') {
    const { runAzureGovern } = await import('./azure/index.js');
    return runAzureGovern(argv.slice(argv.indexOf('azure') + 1));
  }
