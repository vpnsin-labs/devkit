...
export async function requireDep(specifier) {
  try {
    ...
    if (err && (err.code === 'ERR_MODULE_NOT_FOUND' || err.code === 'MODULE_NOT_FOUND')) {
      // Azure-only users just need `yaml` (or can use a .json config); the GitHub
      // commands need the Octokit set.
      const onlyYaml = specifier === 'yaml';
      throw new GovernError(
        `Missing dependency "${specifier}".\n` +
          (onlyYaml
            ? `  Reading a YAML config needs the yaml package (or use governance.config.json). Install it once:\n\n`
            : `  The govern commands need a few extra packages. Install them once:\n\n`) +
          `    npm i -D ${onlyYaml ? 'yaml' : GOVERN_DEPS}\n`
      );
    }
    throw err;
  }
