...
// Hosts the token may be sent to without further configuration. Anything else
// (Azure DevOps Server on-prem) must be listed in azure.trustedHosts so a tampered
// config file or environment cannot redirect the PAT to an arbitrary host.
const AZURE_HOSTS = /^(dev\.azure\.com|[a-z0-9-]+\.visualstudio\.com)$/i;

export function resolveOrgUrl(explicit, { trustedHosts = [] } = {}) {
  const raw =
    explicit ||
  ...
  if (!/^[a-z][a-z0-9+.-]*:\/\//i.test(url)) url = `https://dev.azure.com/${url}`; // bare org name

  let parsed;
  try {
    parsed = new URL(url);
  } catch {
    throw new GovernError(`Invalid Azure DevOps organization URL: ${url}`);
  }
  if (parsed.protocol !== 'https:') {
    throw new GovernError(
      `Azure DevOps organization URL must use https (got ${parsed.protocol}//) — the token travels in the Authorization header.`
    );
  }
  const host = parsed.hostname.toLowerCase();
  const trusted = (trustedHosts ?? []).map((h) => String(h).toLowerCase());
  if (!AZURE_HOSTS.test(host) && !trusted.includes(host)) {
    throw new GovernError(
      `Refusing to send credentials to ${host}: not dev.azure.com or *.visualstudio.com.\n` +
        '  For Azure DevOps Server add the host to azure.trustedHosts in governance.config.yml.'
    );
  }
  return url;
}
